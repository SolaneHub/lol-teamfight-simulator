<template>
  <div class="flex flex-col gap-4">
    ...
    <div :class="[
      ...
      <span>{{ side === 'blue' ? (mode === 'customizer' ? 'Blue Team Builds' : 'Blue Team') : (mode === 'customizer' ? 'Red Team Builds' : 'Red Team') }}</span>
      <span :class="[
    ...
    <div class="flex flex-col gap-3 px-2.5">
      <div v-for="slot in slots" :key="slot.id" @click="onSlotClick(slot)" :class="getSlotClass(slot)">
        <!-- Background Image if champion is selected -->
        <template v-if="slot.champion">
          <img :src="getChampionSplashUrl(slot.champion.id)"
            :class="[
          ...
          <div :class="[
            'absolute inset-0',
            side === 'blue' ? 'bg-linear-to-r from-black/95 via-black/40 to-transparent' : 'bg-linear-to-l from-black/95 via-black/40 to-transparent'
          ]"></div>

          <!-- Clear button (only in browser mode) -->
          <button v-if="mode === 'browser'" @click.stop="unassignSlot(slot)"
            :class="[
        ...
        <div :class="[
          ...
          <span class="text-base text-slate-400 uppercase tracking-widest font-mono font-bold mb-0.5">{{ slot.role }}</span>
          
          <template v-if="slot.champion">
            <span class="text-base font-semibold text-white tracking-wide truncate drop-shadow-md leading-tight">
              {{ slot.champion.name }}
            </span>
            <span v-if="mode === 'customizer'" class="text-base font-mono mt-0.5 block leading-none" :class="side === 'blue' ? 'text-sky-400' : 'text-red-400'">
              Lvl {{ slot.level }}
            </span>
          </template>
...
const getSlotClass = (slot: DraftSlot) => {
  const baseClasses = 'relative h-37.5 rounded-xl overflow-hidden flex items-center transition-all duration-300 select-none group cursor-pointer'
