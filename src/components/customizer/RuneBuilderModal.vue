  ...
  <div v-if="isOpen"
    ...
    <div
      ...
      <div class="relative h-28 flex items-end p-5 border-b border-slate-800 shrink-0">
        <img v-if="activeCustomizerSlot?.champion"
        ...
        <div class="absolute inset-0 bg-linear-to-t from-[#0f1422] via-[#0f1422]/20 to-transparent"></div>
        ...
        <div class="relative z-10 flex items-center justify-between w-full">
          <div class="flex items-center gap-3.5">
            ...
            <div>
              <h3 class="text-lg font-bold text-white tracking-wide leading-none mb-1">Rune Page Customizer</h3>
              <p class="text-base text-slate-400 font-mono">
                Customizing: {{ activeCustomizerSlot?.champion?.name }}
              </p>
            </div>
          </div>
          <button @click="$emit('close')" class="text-slate-400 hover:text-white text-2xl cursor-pointer leading-none px-2 py-1">
            ×
          </button>
        </div>
      </div>
      ...
      <div class="grow overflow-y-auto p-4 flex flex-col gap-4 custom-scrollbar">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:min-h-[440px]">
          ...
          <div class="flex flex-col gap-4 h-full">
            <span class="text-base text-cyan-400 font-bold uppercase tracking-widest font-mono text-center">Primary Path</span>

            <!-- Path Selectors as Beautiful Cards -->
            <div class="flex flex-row items-center gap-2.5 justify-center w-full mx-auto">
              <div v-for="tree in orderedRunes" :key="tree.id" @click="selectPrimaryPath(tree)" :title="tree.name" :class="[
                ...
                <img :src="getRuneIconUrl(tree.icon)"
            ...
            <div v-if="activeCustomizerSlot?.primaryPath" class="grow flex flex-col justify-between w-64 mx-auto relative py-3">
              <!-- Render 4 slots (Keystone, Slot 1, Slot 2, Slot 3) -->
              <div v-for="(slot, slotIdx) in activeCustomizerSlot.primaryPath.slots" :key="slotIdx"
                ...
                <div class="flex flex-row items-center justify-between w-full py-0.5">
                  <div v-for="rune in slot.runes" :key="rune.id" @click="selectPrimaryRune(slotIdx, rune)" @mouseenter="showRuneTooltip(rune, slotIdx === 0 ? 'Primary Keystone' : 'Primary Rune')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" :title="rune.name" :class="[
                    ...
                    <img :src="getRuneIconUrl(rune.icon)"
                      class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                  </div>
                </div>
              </div>
          ...
          <div class="flex flex-col gap-4 lg:border-l lg:border-slate-900/60 lg:pl-4 h-full">
            <span class="text-base text-cyan-400 font-bold uppercase tracking-widest font-mono text-center">Secondary Path</span>
            ...
            <div v-if="activeCustomizerSlot?.secondaryPath" class="grow flex flex-col justify-between w-64 mx-auto relative py-3">
              <!-- Render the 3 normal slots for secondary path -->
              <div v-for="(slot, slotIdx) in activeCustomizerSlot.secondaryPath.slots.slice(1)" :key="slotIdx"
                class="flex flex-col gap-2 relative z-10">
                <!-- List of Rune Choices in a Row (Distributed) -->
                <div class="flex flex-row items-center justify-between w-full py-0.5">
                  <div v-for="rune in slot.runes" :key="rune.id" @click="toggleSecondaryRune(rune)" @mouseenter="showRuneTooltip(rune, 'Secondary Rune')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" :title="rune.name" :class="[
              ...
              <div class="flex flex-row items-center justify-center gap-3.5 py-0.5 relative z-10">
                <!-- Adaptive Force -->
                <button @click="setShardOffensive('adaptive')" @mouseenter="showRuneTooltip('adaptive', 'Offensive Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Adaptive Force (+9)" :class="[
                  ...
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsAdaptiveForceIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
                <!-- Attack Speed -->
                <button @click="setShardOffensive('as')" @mouseenter="showRuneTooltip('as', 'Offensive Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Attack Speed (+10%)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                  activeCustomizerSlot?.shardOffensive === 'as'
                    ? 'bg-amber-500/20 border-amber-500 shadow-md ring-2 ring-amber-500/20'
                    : 'bg-[#0b0e17] border-slate-900 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'
                ]">
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsAttackSpeedIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
                <!-- Ability Haste -->
                <button @click="setShardOffensive('haste')" @mouseenter="showRuneTooltip('haste', 'Offensive Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Ability Haste (+8)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                  activeCustomizerSlot?.shardOffensive === 'haste'
                    ? 'bg-amber-500/20 border-amber-500 shadow-md ring-2 ring-amber-500/20'
                    : 'bg-[#0b0e17] border-slate-900 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'
                ]">
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsCDRScalingIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
              </div>
              ...
              <div class="flex flex-row items-center justify-center gap-3.5 py-0.5 relative z-10">
                <!-- Adaptive Force -->
                <button @click="setShardFlex('adaptive')" @mouseenter="showRuneTooltip('adaptive', 'Flex Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Adaptive Force (+9)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                ...
                <!-- Movement Speed -->
                <button @click="setShardFlex('ms')" @mouseenter="showRuneTooltip('ms', 'Flex Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Movement Speed (+2%)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                  activeCustomizerSlot?.shardFlex === 'ms'
                    ? 'bg-amber-500/20 border-amber-500 shadow-md ring-2 ring-amber-500/20'
                    : 'bg-[#0b0e17] border-slate-900 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'
                ]">
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsMovementSpeedIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
                <!-- Scaling Health -->
                <button @click="setShardFlex('scaling_hp')" @mouseenter="showRuneTooltip('scaling_hp', 'Flex Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Scaling Health (+10-180)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                  activeCustomizerSlot?.shardFlex === 'scaling_hp'
                    ? 'bg-amber-500/20 border-amber-500 shadow-md ring-2 ring-amber-500/20'
                    : 'bg-[#0b0e17] border-slate-900 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'
                ]">
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsHealthScalingIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
              </div>
              ...
              <div class="flex flex-row items-center justify-center gap-3.5 py-0.5 relative z-10">
                <!-- Scaling Health -->
                <button @click="setShardDefensive('scaling_hp')" @mouseenter="showRuneTooltip('scaling_hp', 'Defensive Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Scaling Health (+10-180)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                ...
                <button @click="setShardDefensive('tenacity')" @mouseenter="showRuneTooltip('tenacity', 'Defensive Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Tenacity & Slow Resist (+10%)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                  activeCustomizerSlot?.shardDefensive === 'tenacity'
                    ? 'bg-amber-500/20 border-amber-500 shadow-md ring-2 ring-amber-500/20'
                    : 'bg-[#0b0e17] border-slate-900 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'
                ]">
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsTenacityIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
                <!-- Flat Health -->
                <button @click="setShardDefensive('flat_hp')" @mouseenter="showRuneTooltip('flat_hp', 'Defensive Shard')" @mouseleave="hideRuneTooltip" @mousemove="onMouseMove" title="Health (+65 Flat)" :class="[
                  'group h-9 w-9 rounded-full border flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-[1.1] p-1 shadow-sm shrink-0 bg-[#0b0e17] relative z-10',
                  activeCustomizerSlot?.shardDefensive === 'flat_hp'
                    ? 'bg-amber-500/20 border-amber-500 shadow-md ring-2 ring-amber-500/20'
                    : 'bg-[#0b0e17] border-slate-900 opacity-30 grayscale hover:opacity-100 hover:grayscale-0'
                ]">
                  <img :src="getRuneIconUrl('v1/perk-images/StatMods/StatModsHealthPlusIcon.png')"
                    class="h-full w-full object-contain transition-transform group-hover:scale-110 select-none pointer-events-none" />
                </button>
              </div>
            </div>
...
import { getChampionPosition, getRuneIconUrl, getChampionSplashUrl, getChampionIconUrl } from '@/services'
